...
<body>
    ...
    <div class="sub-container p-regular">
        <h2 class="s-h-imcre24">공지사항</h2>
        <div class="main-search">
            <div class="select-wrap">
                <label class="label-bold" for="searchType">Category</label>
                <select class="p-regular" name="searchType" id="searchType">
                    <option value="bbs_title" <c:if test="${page.searchType eq 'notice_title'}">selected</c:if>>제목
                    </option>
                    <option value="bbs_content" <c:if test="${page.searchType eq 'notice_content'}">selected</c:if>>내용
                    </option>
                    <option value="member_nickname"
                            <c:if test="${page.searchType eq 'member_nickname'}">selected</c:if>>작성자
                    </option>
                </select>
            </div>
        ...
        <div class="board">
            <div class="board-top">
                ...
                <c:if test="${sessionScope.memberGrade == 99}">
                    <button class="btn-Write fill-btn" id="writeButton">공지사항 작성</button>
                </c:if>
            </div>
            <div class="board-list-wrap">
                <c:forEach items="${list}" var="list">
                    <a href="/board/notice/view?notice_id=${list.notice_id}" class="list-box">
                        <div class="left">
                            <div class="left-text">
                                <h5 class="h-pre24">${list.notice_title}</h5>
                                <ul class="p-regular left-bottom">
                                    ...
                                    <li><c:set var="today" value="<%= new java.util.Date() %>"/>
                                        <c:choose>
                                            ...
                                            <c:otherwise>
                                                <!-- 작성일이 오늘이 아닐 경우 -->
                                                <fmt:formatDate value="${list.create_dt}"
                                                                pattern="yyyy년 MM월 dd일 HH:mm"/>
                                            </c:otherwise>
                                        </c:choose>
                                    </li>
                                    <span></span>
                                    <td>조회수 ${list.notice_views}</td>
                                </ul>
                            </div>
                        </div>
                    </a>
        ...
        <div>
            <c:if test="${page.prev}">
                <span>[ <a
                        href="/board/notice/list?num=${page.startPageNum - 1}${page.searchTypeKeyword}">이전</a> ]</span>
            </c:if>
            ...
            <c:if test="${page.next}">
                <span>[ <a href="/board/notice/list?num=${page.endPageNum + 1}${page.searchTypeKeyword}">다음</a> ]</span>
            </c:if>
        </div>
    </div>
    <script>
        document.getElementById("searchBtn").onclick = function () {
            ...
            location.href = "/board/notice/list?num=1" + "&searchType=" + searchType + "&keyword=" + keyword;
        };
